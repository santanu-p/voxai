...
export class GeminiLive {
    constructor(options = {}) {
        ...
        this.relayUrl = options.relayUrl || process.env.NEXT_PUBLIC_RELAY_WS_URL || '';
    ...
    buildRelayUrl() {
        const fromEnv = this.normalizeRelayUrl(this.relayUrl);
        if (fromEnv) {
            return fromEnv;
        }

        const isLikelyVercel = typeof window !== 'undefined' && window.location.hostname.endsWith('.vercel.app');
        if (isLikelyVercel) {
            throw new Error(
                'This Vercel deployment needs NEXT_PUBLIC_RELAY_WS_URL set to an external relay (for example wss://relay.yourdomain.com/api/live).'
            );
        }
        ...
        return new Promise((resolve) => {
            ...
            this.socket.onerror = (event) => {
                ...
                this.onError(
                    normalizeError(
                        event?.error ||
                        'WebSocket connection error. Verify relay URL and server availability.'
                    )
                );
            };
