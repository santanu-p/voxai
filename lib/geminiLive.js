...
function normalizeError(error) {
    ...
    return new Error('Unknown Noa Live error');
}
    ...
    async getEphemeralToken() {
        ...
        try {
            ...
            for (let attempt = 0; attempt <= this.tokenMaxRetries; attempt += 1) {
                ...
                const error = new Error(payload?.error || 'Failed to create Noa session token');
                lastError = error;
            ...
            throw lastError || new Error('Failed to create Noa session token');
        } finally {
